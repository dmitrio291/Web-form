...
// Если заполненны все поля формы тогда готовим письмо и отправляем
if ( !empty($_POST) && trim($_POST['name']) != '' && trim($_POST['email']) != '' && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) && trim($_POST['message']) != ''  ) {
    ...
    // Формируем тему письма, специально обрабатывая ее
    $subject = "=?utf-8?B?".base64_encode("Сообщение с сайта!")."?=";

    // Указываем от кого  будет отправлено письмо
    $email_from = "info@webdev.ru";
    ...
    $sendResult = mail( 'dmitry@test.ru', $subject, $message, $headers );

    if ( $sendResult ) {
        // Перенапрвялем на страницу "Спасибо"
...
// Проверка переменной на ее наличии и на заполненность
